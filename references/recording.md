...
For gist-linked PR playback, follow the explicit consent and publication steps in SKILL.md. Export with `--compress --out <temporary-directory>/replay.json` and upload only `replay.json`, verify the gist is secret, pin its revision, and append one `chronosphere-replay: <gist-url>` line to the PR description. The compressed gist envelope is limited to 10 MB and expanded JSONL to 20 MB. The decoder caps streamed expansion and verifies the declared size and SHA-256 hash before normal recording validation. Legacy plain `replay.jsonl` is also supported; include only one replay file in a gist. Anyone with the link can read a secret gist.
